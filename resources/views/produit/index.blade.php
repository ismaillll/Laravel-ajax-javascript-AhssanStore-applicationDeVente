    ...
    <table class="table table-bordered  table-striped" id="table">
      ...
      @foreach ($produit as $value)
        <tr >
          ...
          <td style="color:blue"><a class="ui yellow label">Presque Fini</td>
          @endif
          @if(empty($value->categorie->id))
          <td >pas de catégorie</td>
          @else

         <td ><a class="ui basic label">{{ $value->categorie->nom }}</a></td>
          @endif
          <td>{{ $value->prix }}.DH</td>
          @if(!empty($value->marque->logo))
          <td><img src='{{ asset('public/images/'.$value->marque->logo) }}' style="width:70px; height:35px; float:left; border-radius:00%; margin-right:00px;"></td>

          @else

          <td ><a class="ui  label">{{ $value->marque->nom }}</a></td>
          @endif
          <td ><a class="ui grey label">{{ $value->fournisseur->nom}}</a></td>
