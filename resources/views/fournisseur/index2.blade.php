      ...
      @foreach ($four as $value)
        <tr >
          ...
          <td>
            ...
            <a href="#" class=" btn btn-warning btn-sm" data-fourid="{{$value->id}}" data-toggle="modal" data-target="#edit" data-nom="{{$value->nom}}" data-email="{{$value->email}}" data-telephone="{{$value->telephone}}" data-fax="{{$value->fax}}" data-ville="{{$value->ville}}" data-adresse1="{{$value->adresse1}}"  data-adresse2="{{$value->adresse2}}">
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="{{route('updateFournisseur')}}" method="post">
          {{method_field('patch')}}
      		{{csrf_field()}}
	      <div class="modal-body">
	      		<input type="hidden" name="fourid" id="fourid" value="">
